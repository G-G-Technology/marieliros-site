<template>
  <section class="grid grid-cols-8 gap-1 md:grid-cols-12 md:gap-4 pt-16 md:pt-24">
    <div class="col-start-2 col-span-6 md:col-start-2 md:col-span-10 flex flex-col items-start">
      ...
      <div class="text-dark-brown font-abhayaLibre text-lg md:text-xl text-justify py-3 whitespace-pre-wrap">
        {{ therapy?.gestalt || defaultTherapy.gestalt }}
      </div>

      <div class="text-dark-brown font-cinzel text-xl md:text-3xl font-bold py-2">SOBRE A PSICOTERAPIA</div>
      <div class="text-dark-brown font-abhayaLibre text-lg md:text-xl text-justify pt-3 whitespace-pre-wrap">
        {{ therapy?.psicoterapia || defaultTherapy.psicoterapia }}
      </div>
    </div>
  </section>
</template>
...
const props = defineProps<{
  therapy?: { gestalt: string; psicoterapia: string } | null;
}>();
...
// suppress unused-vars warning from vue-tsc; props is used via template
void props;
</script>
